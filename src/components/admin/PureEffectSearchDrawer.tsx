...
interface PureEffectItemDetail {
  external_id: string;
  shopify_id: number;
  title: string;
  handle: string;
...
interface PureEffectSearchDrawerProps {
  open: boolean;
...
export function PureEffectSearchDrawer({
  open,
  onOpenChange,
  onImported,
}: PureEffectSearchDrawerProps) {
  const importMutation = useImportToProduct();

  const [keywords, setKeywords] = useState("");
  const [includeUnavailable, setIncludeUnavailable] = useState(true);
  const [isSearching, setIsSearching] = useState(false);
  const [searchResults, setSearchResults] = useState<PureEffectItem[]>([]);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (!data.items || data.items.length === 0) {
        console.warn("[PureEffectImportWarning]", JSON.stringify({
          event: "PureEffectImportWarning",
          endpoint_status: "ok",
      ...
      console.error("Pure Effect search error:", error);
  ...
  const fetchItemDetails = async (
    handle: string
  ): Promise<PureEffectItemDetail | null> => {
    try {
      const { data, error } = await supabase.functions.invoke(
        "pureeffect-item",
        { body: { handle } }
      ...
      if (error) {
        console.error("pureeffect-item error:", error);
        return null;
      }
      return data?.item || null;
    } catch (err) {
      console.error("pureeffect-item exception:", err);
      return null;
    }
  };
    ...
    try {
      for (let i = 0; i < itemsToImport.length; i++) {
        if (imported >= MAX_IMPORT_PER_RUN) {
          ...
          console.info(
            `[Pure Effect Import] Run limit reached (${MAX_IMPORT_PER_RUN}). Stopping.`
          );
          toast.warning(
        ...
        if (!detail) {
          console.warn(
            `[Pure Effect Import] Could not fetch details for ${handle} — skipping`
          );
          failed++;
          errorCount++;
          continue;
        ...
        const parsed = parseListingFields({
          ...
          apiBrand: detail.vendor || undefined,
        ...
        const description =
          detail.description ||
          `${detail.title}. ${detail.vendor}${detail.size ? `, Size: ${detail.size}` : ""}${detail.material ? `, Material: ${detail.material}` : ""}`;
        ...
        const importInput = {
          ...
          brand: parsed.brand_text || detail.vendor || "Pure Effect",
          ...
          // Invariant: condition is always null for Pure Effect
          condition: null,
          price: detail.price,
        ...
        try {
          ...
          console.error(`[Pure Effect Import] Failed to import ${handle}:`, importErr);
          failed++;
          errorCount++;
          continue;
        }
        ...
        if (!detail.available) {
          soldOut++;
          console.info(
            `[Pure Effect Import] ${handle} is sold out — created as draft with note`
          );
        }
      ...
      console.info("[Pure Effect Import Run]", JSON.stringify({
      ...
      console.error("[Pure Effect Import Run] Fatal error:", JSON.stringify({
        importer_name: "pure_effect",
        endpoint_status: "error",
        products_imported: imported,
        duration_ms: durationMs,
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full sm:max-w-xl flex flex-col">
        <SheetHeader>
          ...
          <SheetDescription>
            Browse Pure Effect Sweden catalog and import items as draft products.
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="flex-1 flex flex-col gap-6 mt-6 overflow-hidden">
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <Label htmlFor="pe-unavailable" className="text-sm">
