...
BEGIN
  ...
  -- tradera-sync: once daily at 03:00 UTC (was every 2h)
  ...
  -- tradera-retry-import: keep every 30 min (quota guard in function)
  PERFORM cron.schedule(
  ...
  -- ebay-availability: keep every 2h
  PERFORM cron.schedule(
  ...
  RETURN jsonb_build_object('status', 'ok', 'message', 'Cron jobs re-registered: tradera-sync daily 03:00, retry every 30m, ebay every 2h');
END;
$function$;